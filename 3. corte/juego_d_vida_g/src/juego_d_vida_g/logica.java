...
public class logica extends JApplet {
    ...
    private int Duration= 0; //Tiempo de retardo de cada cambio de pantalla
    ...
    public void paint(Graphics g) {
        ...
        juego automata = new juego();

        //Mientras se abrió la aplicación
        while (true) {
            int[][] M = automata.juego(ca_lenght); //Generar una nueva matriz de autómatas celulares
            ...
            for (int i = 0; i < ca_lenght; i++) {  //Mueva todos los autómatas celulares              
                ...
                for (int j = 0; j < ca_lenght; j++) {
                    if (M[i][j] == 0) {
                        ...
                        g2.setPaint(Color.YELLOW);//La posición en la matriz es una, pintar el cuadrado de color
                    }
